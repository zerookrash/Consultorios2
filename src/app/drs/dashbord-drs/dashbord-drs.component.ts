...
export class DashbordDrsComponent implements OnInit {
  ...
  constructor(
   ...
   onRol(){
    ...
    let id = this.user.id;
    let urlApi = `http://localhost:8061/api/cat_usuarios/${id}/citas?access_token=${token}`;
   ...
   onCita(){
    ...
    let token = localStorage.getItem("accessToken");
    let urlApi = `http://localhost:8061/api/cat_citas2s/reservas-hoy?fecha=${this.reg_cita.fechaCita}&access_token=${token}`;
  ...
  onDelete(id, fk) {
    let token = localStorage.getItem("accessToken");
    let urlCita = `http://localhost:8061/api/cat_usuarios/${id}/citas/${fk}?access_token=${token}`;
    this.http.get(urlCita).subscribe( (res:CitaInterface) => {
      ...
      if(citaFecha.diff(hoy, 'hours') < 24 || res.estadoPago == 'PAGADO'){
        ...
        let urlApi = `http://localhost:8061/api/cat_usuarios/${id}/citas/${fk}?access_token=${token}`;
